	...
	Describe("List()", func() {
		Context("When there are multiple quota definitions", func() {
			BeforeEach(func() {
				...
				server.AppendHandlers(
					ghttp.CombineHandlers(
						ghttp.VerifyRequest(http.MethodGet, "/v2/quota_definitions"),
								...
								"resource_quotas": [
									{
										...
										"resource_id": "rcf0c17db8-35ad-11e7-a919-92ebcb67fe33",
										"limit": 1
									},
									{
										"_id": "8b35eb93b6b14f50b6c4f3ff0dfd0358",
										"resource_id": "rcd1a5a758-03a1-11e7-93ae-92361f002671",
										"limit": 1
									}
								],
			...
			It("should return all of them", func() {
				...
				Expect(definition.ResourceQuotas).Should(HaveLen(2))
				...
				resourceQuota := definition.ResourceQuotas[0]
				Expect(resourceQuota.ID).Should(Equal("65012f5b6fa84ecaaac5eab4abc2d0fd"))
				Expect(resourceQuota.ResourceID).Should(Equal("rcf0c17db8-35ad-11e7-a919-92ebcb67fe33"))
				Expect(resourceQuota.Limit).Should(Equal(1))
				resourceQuota = definition.ResourceQuotas[1]
				Expect(resourceQuota.ID).Should(Equal("8b35eb93b6b14f50b6c4f3ff0dfd0358"))
				Expect(resourceQuota.ResourceID).Should(Equal("rcd1a5a758-03a1-11e7-93ae-92361f002671"))
				Expect(resourceQuota.Limit).Should(Equal(1))
				...
				Expect(definition.ResourceQuotas).Should(BeEmpty())
				Expect(definition.CreatedAt).Should(Equal("2017-05-16T17:12:52.925Z"))
				Expect(definition.UpdatedAt).Should(Equal("2017-05-16T17:12:52.925Z"))
			})
		})
	...
	Describe("Get()", func() {
		Context("When the quota definition exists", func() {
			BeforeEach(func() {
				...
				server.AppendHandlers(
					ghttp.CombineHandlers(
						ghttp.VerifyRequest(http.MethodGet, "/v2/quota_definitions/a3d7b8d01e261c24677937c29ab33f3c"),
						ghttp.RespondWith(http.StatusOK, `{
							"name": "Pay-as-you-go Quota",
							"type": "SDQ",
							"number_of_service_instances": 3,
			...
			It("should return the quota definition", func() {
				...
				Expect(quota.ResourceQuotas).Should(HaveLen(2))
				resourceQuota := quota.ResourceQuotas[0]
				Expect(resourceQuota.ID).Should(Equal("65012f5b6fa84ecaaac5eab4abc2d0fd"))
				Expect(resourceQuota.ResourceID).Should(Equal("rcf0c17db8-35ad-11e7-a919-92ebcb67fe33"))
				Expect(resourceQuota.Limit).Should(Equal(1))
				resourceQuota = quota.ResourceQuotas[1]
				Expect(resourceQuota.ID).Should(Equal("8b35eb93b6b14f50b6c4f3ff0dfd0358"))
				Expect(resourceQuota.ResourceID).Should(Equal("rcd1a5a758-03a1-11e7-93ae-92361f002671"))
				Expect(resourceQuota.Limit).Should(Equal(1))
